...
import tasks.task1_2_lp.document_template.matrix_symplex.util.latex as ms_latex
import tasks.task1_2_lp.document_template.matrix_symplex.util.matrices as ms_matrices
import tasks.task1_2_lp.document_template.matrix_symplex.util.elements as ms_elements
from report.docx.pretty_omml import sympy_matrix_to_omml, num_as_str
...
def P_equation(step_data: MatrixSymplexStepData):
    return [
...
def P_inv_equation(step_data: MatrixSymplexStepData):
    return [
...
def delta_equation(step_data: MatrixSymplexStepData, free_var_index: int):
...
def basis_value_expression(step_data: MatrixSymplexStepData):
...
def Z_column_expression(step_data: MatrixSymplexStepData):
    P_inv = ms_matrices.P_inv(step_data.current_solution)
...
def basis_exclude_criteria_expression(step_data: MatrixSymplexStepData):
    result = []
    ...
    for i, b in enumerate(basis):
        ...
        if z_element_value > 0:
            data = [
                ...
                f" = \\frac{{{num_as_str(basis_element_value)}}}{{{num_as_str(z_element_value)}}} =",
                f"{num_as_str(basis_element_value / z_element_value)}"
            ]
        else:
...
def F_expression(step_data: MatrixSymplexStepData):
