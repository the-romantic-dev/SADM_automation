...
def optimize(f, start):
    x1, x2 = sp.symbols("x1 x2")
    initial_guess = np.array(start.T.tolist()[0])
    np_func = sp.lambdify((x1, x2), -f, 'numpy')

    def obj(x: np.ndarray):
        return np_func(x[0], x[1])
    # f_num = sp.lambdify([(x1, x2)], -f, "scipy")
    # initial_guess = np.zeros(2)
    # initial_guess[0] = start[0, 0]
    # initial_guess[1] = start[1, 0]
    result = minimize(fun=obj, x0=initial_guess, method="Nelder-Mead")
