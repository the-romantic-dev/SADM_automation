...
def normalized_float_latex(f_num: float):
    ...
    # return result
    def round_float(number, decimals=0):
        """
        Округляет число number до заданного количества цифр после запятой decimals.
        Если результат округления равен 0, то округляет до ближайшей значащей цифры.
        """
        rounded = round(number, decimals)
        if rounded == 0:
            # Если округленное значение равно 0, то округляем до ближайшей значащей цифры
            nonzero_digits = len(str(int(1 / (10 ** (decimals + 1)))))
            return round(number, -nonzero_digits)
        else:
            return rounded

    return f"{round_float(f_num, decimals=4)}"
